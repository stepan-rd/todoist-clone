...
export default function PriorityDropdown({ className }: Props) {
  const { appColors } = useAppColors();

  const { setPriorityDropdownVisible } = useAddTaskStore();

  return (
    <>
      <OverlayInvisible onClick={() => setPriorityDropdownVisible(false)} />
      <div
        className={`rounded-lg shadow-md z-50 lg py-1 border hover:cursor-pointer ${className}`}
        style={{
      ...
      >
        {choices.map((choice, choiceIndex) => (
          <PriorityChoice
            flagSmaller={smallerFlagSvgs[choiceIndex]}
